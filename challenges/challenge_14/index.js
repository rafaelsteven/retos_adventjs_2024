...
function minMovesToStables(reindeer, stables) {
    let min = 0;    
    for (const reno of reindeer) {
      let distance = null;
      let stablesArray = 0;
      for (let j = 0; j < stables.length; j++) {
        let calcular = Math.abs(reno - stables[j]) * + 1;
        if (distance > calcular || distance === null) {
           distance = calcular;
           stablesArray = j;
        }
      }
      stables.splice(stablesArray, 1);
      min += distance;
    }
    return min
  }
  ...
  console.log(minMovesToStables([30, 1, 2], [1, 2, 30]));